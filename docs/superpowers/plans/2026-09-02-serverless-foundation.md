...
The guard, the try/catch and the `finally` are not optional. Without them an
unhandled `pg` rejection prints a stack trace built from a `Client` holding the
full connection string — password included — into whatever terminal or CI log
